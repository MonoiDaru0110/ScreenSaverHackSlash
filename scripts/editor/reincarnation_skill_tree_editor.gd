...
const SAVE_PATH = "res://data/reincarnation_skills.json"
...
func _ready() -> void:
	...
	var add_btn: Button = $VBoxContainer/HBoxContainer/AddBtn
	var copy_btn: Button = $VBoxContainer/HBoxContainer/CopyBtn
	var paste_btn: Button = $VBoxContainer/HBoxContainer/PasteBtn
	var save_btn: Button = $VBoxContainer/HBoxContainer/SaveBtn
	var load_btn: Button = $VBoxContainer/HBoxContainer/LoadBtn

	add_btn.pressed.connect(_on_add_button_pressed)
	copy_btn.pressed.connect(copy_selected_node)
	paste_btn.pressed.connect(paste_skill_data)
	save_btn.pressed.connect(save_data)
	load_btn.pressed.connect(load_data)
...
func add_new_node(id := "", name_str := "", icon := "❓", desc := "", cost := 1, pos := Vector2(100, 100)) -> GraphNode:
	...
	# Override styles to merge titlebar and panel, making it a unified small square with purple tint
	var flat_box = StyleBoxFlat.new()
	flat_box.bg_color = Color(0.18, 0.12, 0.28, 1.0)
	flat_box.border_width_left = 2
	flat_box.border_width_top = 2
	flat_box.border_width_right = 2
	flat_box.border_width_bottom = 2
	flat_box.border_color = Color(0.5, 0.3, 0.7, 1.0)
	...
	flat_box_selected.border_color = Color(0.9, 0.7, 1.0, 1.0)
	flat_box_selected.bg_color = Color(0.25, 0.16, 0.38, 1.0)
	...
	# Left (input/prereq) is Cyan (Color(0.2, 0.7, 1.0)), Right (output/unlocks) is Purple/Pink (Color(0.9, 0.5, 1.0))
	node.set_slot(0, true, 0, Color(0.2, 0.7, 1.0), true, 0, Color(0.9, 0.5, 1.0))
...
func _on_add_button_pressed() -> void:
	...
	var n = 1
	var new_id = "reinc_skill_%d" % n
	while new_id in used_ids:
		n += 1
		new_id = "reinc_skill_%d" % n
...
func _update_node_icon_preview(node: GraphNode, icon_val: String) -> void:
	...
	lbl.text = icon_val if not icon_val.is_empty() else "⚛️"
	lbl.visible = true
